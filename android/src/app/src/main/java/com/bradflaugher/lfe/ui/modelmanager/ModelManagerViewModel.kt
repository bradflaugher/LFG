...
import com.bradflaugher.lfe.BuildConfig
import com.bradflaugher.lfe.R
import com.bradflaugher.lfe.common.ProjectConfig
import com.bradflaugher.lfe.common.SystemPromptHelper
import com.bradflaugher.lfe.common.getJsonResponse
...
import com.bradflaugher.lfe.data.CategoryInfo
import com.bradflaugher.lfe.data.Config
...
import com.bradflaugher.lfe.data.EMPTY_MODEL
import com.bradflaugher.lfe.data.IMPORTS_DIR
import com.bradflaugher.lfe.data.Model
import com.bradflaugher.lfe.data.ModelAllowlist
...
import com.bradflaugher.lfe.data.SOC
import com.bradflaugher.lfe.data.SystemPromptRepository
import com.bradflaugher.lfe.data.TMP_FILE_EXT
...
import com.bradflaugher.lfe.data.ModelFile
import com.bradflaugher.lfe.data.createLlmChatConfigs
import com.bradflaugher.lfe.proto.AccessTokenData
import com.bradflaugher.lfe.proto.ImportedModel
import com.bradflaugher.lfe.proto.Theme
...
private const val ALLOWLIST_BASE_URL =
  "https://raw.githubusercontent.com/bradflaugher/LFE/refs/heads/main/android/src/app/src/main/assets/model_allowlists"
...
open class ModelManagerViewModel
  ...
  constructor(
    ...
    val customTasks: Set<@JvmSuppressWildcards CustomTask>,
    ...
    /**
     * Download a model directly from HuggingFace, given a repo + the specific runtime file
     * the user picked. The model is added to the agent-skills task's models list, given a default
     * LiteRT config, and the existing [downloadModel] worker is kicked off.
     */
    fun downloadFromHuggingFace(
      ...
      val name = "${repoId.substringAfter('/')}-$fileName"
      val url = "https://huggingface.co/$repoId/resolve/main/$fileName?download=true"
      val configs =
        createLlmChatConfigs(
          defaultTopK = DEFAULT_TOPK,
          defaultTopP = DEFAULT_TOPP,
          defaultTemperature = DEFAULT_TEMPERATURE,
          defaultMaxToken = 1024,
          defaultMaxContextLength = null,
          accelerators = listOf(Accelerator.GPU, Accelerator.CPU),
          supportThinking = false,
          supportSpeculativeDecoding = false,
        ).toMutableList()
      ...
      val model =
        Model(
          name = name,
          version = "main",
          info = "Downloaded from [$repoId](https://huggingface.co/$repoId).",
          url = url,
          sizeInBytes = sizeInBytes,
          downloadFileName = fileName,
          configs = configs,
          learnMoreUrl = "https://huggingface.co/$repoId",
          ...
          accelerators = listOf(Accelerator.GPU, Accelerator.CPU),
          latestModelFile = ModelFile(fileName = fileName, commitHash = "main"),
        ).also { it.preProcess() }

      val agentTask = getTaskById(BuiltInTaskId.LLM_AGENT_CHAT)
      if (agentTask != null) {
        val existingIdx = agentTask.models.indexOfFirst { it.name == name }
        if (existingIdx >= 0) agentTask.models.removeAt(existingIdx)
        agentTask.models.add(model)
        agentTask.updateTrigger.value = System.currentTimeMillis()
      }

      val modelDownloadStatus = uiState.value.modelDownloadStatus.toMutableMap()
      val modelInitStatus = uiState.value.modelInitializationStatus.toMutableMap()
      modelDownloadStatus[model.name] =
        ModelDownloadStatus(status = ModelDownloadStatusType.IN_PROGRESS)
      modelInitStatus[model.name] =
      ...
      downloadModel(task = agentTask, model = model)
    }
    ...
    fun addImportedLlmModel(info: ImportedModel) {
      ...
      val setOfTasks = mutableSetOf(BuiltInTaskId.LLM_AGENT_CHAT)
    ...
    private fun readModelAllowlistFromDisk(fileName: String = MODEL_ALLOWLIST_FILENAME): ModelAllowlist? {
      ...
      // Fallback: ship a bundled allowlist in assets so first-launch works offline.
      return try {
        context.assets.open(MODEL_ALLOWLIST_FILENAME).bufferedReader().use { reader ->
          Gson().fromJson(reader.readText(), ModelAllowlist::class.java)
        }
      } catch (e: Exception) {
